...
class ImpactTracker:
    def __init__(self):
        ...
        self._last_explicit_ask_date: Optional[str] = None  # evita di chiedere feedback più di 1x/giorno
    ...
    def should_ask_explicit_feedback(self) -> Optional[str]:
        """
        Restituisce il preview dell'azione su cui chiedere feedback esplicito, se pertinente.
        Criteri: azione significativa non ancora valutata + non ha già chiesto oggi.
        """
        today = datetime.now().strftime("%Y-%m-%d")
        if self._last_explicit_ask_date == today:
            return None
        if self._pending_id is None:
            return None
        entry = next((e for e in self._log if e["id"] == self._pending_id), None)
        if not entry or entry.get("impact") is not None:
            return None
        if entry.get("action_type") == "reminder":
            return None
        if len(entry.get("content", "")) < 80:
            return None
        self._last_explicit_ask_date = today
        return entry["content"][:120]

    def mark_ignored(self) -> None:
        """Marca l'azione pendente come ignorata (neutral) se non ha ricevuto risposta dopo 90 minuti."""
        if self._pending_id is None:
            return
        entry = next((e for e in self._log if e["id"] == self._pending_id), None)
        if entry and entry.get("impact") is None:
            entry["impact"] = "neutral"
            entry["response"] = "[ignorato]"
            self._save()
    ...
    def evaluate_response(self, user_message: str, brain=None) -> None:
        ...
        try:
            ...
            entry["response_time_seconds"] = int((datetime.now() - sent_at).total_seconds())
        except Exception:
            pass
        ...
        # Aggiorna feedback_weights.json per tipo di azione proattiva
        try:
            score = 1.0 if impact == "positive" else (-0.5 if impact == "negative" else 0.0)
            ffile = Config.MEMORY_DIR / "feedback_weights.json"
            weights = json.loads(ffile.read_text(encoding="utf-8")) if ffile.exists() else {}
            key = entry.get("action_type", "proactive")
